...
class EqualizerViewModel(private val repository: EqualizerRepository) : ViewModel() {
    ...
    repository.userPresets
        .onEach { presets ->
          ...
          if (!presetRestored) {
            ...
            _preset.value =
                _presets.value.find { bandGains == it.bandGains } ?: Preset(bandGains = bandGains)
  ...
  fun createNewPreset(name: String): PresetNameValidationError? {
    ...
    _preset.value = _preset.value.copy(name = name.trim(), isUserDefined = true, isMutated = false)
    return null
  }
